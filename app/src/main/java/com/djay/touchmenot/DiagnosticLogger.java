...
public final class DiagnosticLogger {
    private static final String TAG = "TouchMeNot-Diag";
    ...
    private static void initOnce() {
        if (inited.compareAndSet(false, true)) {
            String[] paths = new String[]{
                "/sdcard/Download/touchmenot_classes.log",
                Environment.getExternalStorageDirectory() + "/Download/touchmenot_classes.log",
                "/storage/emulated/0/Download/touchmenot_classes.log",
                "/data/local/tmp/touchmenot_classes.log",
                "/sdcard/Documents/touchmenot_classes.log"
            };
            ...
            for (String path : paths) {
                try {
                    ...
                    activePath = path;
                    Log.i(TAG, "DiagnosticLogger initialized at: " + path);
                    return;
                } catch (Throwable t) {
                    Log.w(TAG, "Failed to init diagnostic logger at " + path + ": " + t.getMessage());
                }
            }
            Log.e(TAG, "DiagnosticLogger failed to initialize at any path");
            writer = null;
        }
    }
    ...
    public static void log(String category, String message) {
        try {
            ...
            Log.e(TAG, "Diagnostic log write failed: " + t.getMessage());
            writer = null;
        }
    }
}
